...
function checkCostSpansHaveProviderAndModel(spans: ReadonlyArray<unknown>): void {
    spans.forEach((span, i) => {
        if (span === null || typeof span !== 'object' || Array.isArray(span)) {
        ...
        if (typeof s.provider !== 'string' || s.provider.length === 0) {
            throw new Error(
                `spans[${i}].provider must be a non-empty string — spans without ` +
        ...
        if (typeof s.model !== 'string' || s.model.length === 0) {
            throw new Error(
                `spans[${i}].model must be a non-empty string — spans without ` +
