...
export interface BackingClass {
    readonly apiClass: string;   // e.g. "EventsApi" — must match an actual generated module
 ...
 */
export const SUBDOMAIN_MAP: Readonly<Record<Backend, string>> = {
    bff:   'api',
    meter: 'meter',
    arc:   'arc',
    acute: 'acute',
 ...
 */
export const REGION_INGEST_MAP: Readonly<Record<string, string>> = {
    'us-east-1':      'us',
    'us-west-2':      'us',
    'ap-southeast-1': 'ap',
    'eu-west-1':      'eu',
 ...
 */
export const CAPABILITY_MAP: Readonly<Record<string, readonly BackingClass[]>> = {
    usage: [
...
function validateRouting(): void {
    const validBackends = new Set(Object.keys(SUBDOMAIN_MAP));
    const capSet = new Set(Object.keys(CAPABILITY_MAP));
    const orderSet = new Set(CAPABILITY_ORDER);

    if (capSet.size !== orderSet.size) {
        throw new Error(
            `CAPABILITY_MAP (${capSet.size}) vs CAPABILITY_ORDER (${orderSet.size}) size drift`
        );
    }
    ...
    for (const [capability, classes] of Object.entries(CAPABILITY_MAP)) {
        if (classes.length === 0) {
            throw new Error(
                `capability ${capability} has no backing classes — every namespace must back at least one class`
            );
        }
        for (const bc of classes) {
            if (!validBackends.has(bc.backend)) {
                throw new Error(
                    `capability ${capability} class ${bc.apiClass} declares backend ${bc.backend!}` +
