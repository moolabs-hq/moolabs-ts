...
class WebhookVerifier {
    ...
    verify(payload, signature, algorithm = 'sha256') {
        ...
        const expected = (0, crypto_1.createHmac)(algorithm, this.secret).update(payloadBuf).digest('hex');
        // Constant-time compare; require equal lengths up front to avoid
        // timingSafeEqual throwing on mismatched lengths.
        if (expected.length !== signature.length) {
            return false;
        }
        return (0, crypto_1.timingSafeEqual)(Buffer.from(expected, 'utf-8'), Buffer.from(signature, 'utf-8'));
    }
    /**
