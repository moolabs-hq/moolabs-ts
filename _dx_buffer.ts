...
export const DEFAULT_INGEST_BUFFER_CONFIG: IngestBufferConfig = {
    maxSize: 1000,
...
export class IngestBuffer<T> {
    private readonly drain: DrainCallback<T>;
    ...
    private readonly logger: (msg: string, fields: Record<string, unknown>) => void;
    ...
    constructor(opts: {
        ...
        const config = { ...DEFAULT_INGEST_BUFFER_CONFIG, ...(opts.config ?? {}) };
        if (config.maxSize <= 0) throw new Error(`maxSize must be positive, got ${config.maxSize}`);
        if (config.flushIntervalSec <= 0) throw new Error(`flushIntervalSec must be positive, got ${config.flushIntervalSec}`);
        if (config.shutdownFlushTimeoutSec < 0) throw new Error(`shutdownFlushTimeoutSec must be non-negative, got ${config.shutdownFlushTimeoutSec}`);
        ...
        this.config = config;
        this.logger = opts.logger ?? (() => { /* noop default */ });
    }
    ...
    start(): void {
        ...
        this.timer = setInterval(
            () => { void this.drainOnce(); },
            this.config.flushIntervalSec * 1000,
        );
    }
    ...
    enqueue(events: T[]): void {
        ...
        if (overflow > 0) {
            if (this.config.overflow === OVERFLOW_RAISE) {
                throw new IngestBufferFull(
                    `ingest buffer full at ${this.queue.length} events (maxSize=${this.config.maxSize}); ${events.length} event(s) rejected`
                );
            }
            ...
            this.logger('moolabs.ingestBuffer.overflowDropOldest',
     ...
     *  timeout expires. */
    async close(timeoutSec?: number): Promise<void> {
        if (this.stopped) {
            return;
        }
        this.stopped = true;
        ...
        const limit = timeoutSec ?? this.config.shutdownFlushTimeoutSec;
        ...
        if (this.queue.length > 0) {
            ...
            this.logger('moolabs.ingestBuffer.eventsAbandonedOnShutdown',
    ...
    private async drainOnce(): Promise<void> {
        if (this.inFlight) return;             // never overlap two drain calls
        ...
        try {
            ...
            const batch: T[] = this.queue.splice(0, this.queue.length);

            let delivered = 0;
            let drainFailed = false;
            try {
                delivered = await this.drain(batch);
            } catch {
                drainFailed = true;
            }
            ...
            if (undelivered.length > 0) {
                const newTail: T[] = this.queue.splice(0, this.queue.length);
